...
func pokemonStatsToReduced(ps PokemonStats) PokemonStatsReduced {
	...
	psr.stats.defense = uint8(ps.Stats[2].BaseStat)
	psr.stats.special_attack = uint8(ps.Stats[3].BaseStat)
	psr.stats.special_defense = uint8(ps.Stats[4].BaseStat)
	psr.stats.speed = uint8(ps.Stats[5].BaseStat)
	//checking size before assigning to empty string
	if len(ps.Types) == 2 {
...
func (c *Client) ListPokemonAttributes(pokemon string) (PokemonStats, error) {
	...
	if err := json.Unmarshal(body, &pokemon_stats); err != nil {
		return PokemonStats{}, err
	}

	return pokemon_stats, nil
}
